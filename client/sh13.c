...
int joueurElimine[4] = {0, 0, 0, 0}; // 1 si le joueur est éliminé


int leveMainClients[4] = {0, 0, 0, 0};  // 1 si le joueur i lève la main, 0 sinon
...
int personnages_objets[13][8] = {
    {0,0,1,0,0,0,0,1}, // Sebastian Moran
    {0,1,0,0,0,1,0,1}, // Irene Adler
    {0,0,0,1,1,0,1,0}, // Inspector Lestrade
    {0,0,1,1,1,0,0,0}, // Inspector Gregson
    {0,1,0,1,0,0,0,0}, // Inspector Baynes
    {0,0,1,1,0,0,0,0}, // Inspector Bradstreet
    {1,0,0,1,0,0,1,0}, // Inspector Hopkins
    {1,1,1,0,0,0,0,0}, // Sherlock Holmes
    {1,0,1,0,0,0,1,0}, // John Watson
    {1,1,0,0,1,0,0,0}, // Mycroft Holmes
    {1,0,0,0,0,1,0,0}, // Mrs. Hudson
    {0,0,0,0,1,1,0,0}, // Mary Morstan
    {0,1,0,0,0,0,0,1}  // James Moriarty
};
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				// Si on clique sur "Quitter" à la fin du jeu
				if (jeuTermine && mx >= 412 && mx <= 612 && my >= 400 && my <= 460) {
					quit = 1;
					break;
				}
		...
		switch (gbuffer[0])
		{
			...
			case 'D':
				...
				// Initialiser la ligne de tableCartes de ce joueur à 0
				for (int j = 0; j < 8; j++)
					tableCartes[gId][j] = 0;

				// Remplir la ligne de symboles pour les cartes reçues
				for (int k = 0; k < 3; k++) {
					int carte = b[k];
					for (int symb = 0; symb < 8; symb++) {
						if (personnages_objets[carte][symb])
							tableCartes[gId][symb]++;
					}
				}

				printf("Table cartes du joueur %d :\n", gId);
				for (int symb = 0; symb < 8; symb++) {
					printf("Symbole %d = %d\n", symb, tableCartes[gId][symb]);
				}
			...
			case 'M':
				...
				goEnabled = (joueurCourant == gId && joueurElimine[gId] == 0);

				
				printf("C’est %s qui joue. goEnabled = %d\n", gNames[joueurCourant], goEnabled);


				break;

			// Message 'O' : enquête globale sur un objet
			case 'O': {
				int demandeur, symbole;
				sscanf(gbuffer, "O %d %d", &demandeur, &symbole);

				// Si ce n’est PAS moi qui ai posé la question
				for (int i = 0; i < 4; i++)
				leveMainClients[i] = 0;

				if (gId != demandeur && tableCartes[gId][symbole] > 0) {
					
					leveMainClients[gId] = 1;
					printf("Je lève la main pour l’objet %d\n", symbole);
				}

				printf("Message O reçu : demandeur=%d, symbole=%d\n", demandeur, symbole);
				break;
			}
			...
			//Message  'X' : élimination d'un joueur
			case 'X': {
				int idelimine;
				sscanf(gbuffer, "X %d", &idelimine);
				joueurElimine[idelimine] = 1;
				printf("Le joueur %s est éliminé !\n", gNames[idelimine]);
				break;
			}

			// Messafe 'F': fin de la partie
			case 'F':
				sscanf(gbuffer, "F %[^\n]", messageFin); // Copie le message
				printf(">>> %s\n", messageFin);
				jeuTermine = 1;
				break;
	...
	for (i=0;i<4;i++)
		if (strlen(gNames[i])>0)
		{
			...
			if (leveMainClients[i]) {
				SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255); // Vert
				SDL_Rect rectMain = { 180, 110 + i*60, 10, 10 };  // petit carré vert
				SDL_RenderFillRect(renderer, &rectMain);
			}

			if (joueurElimine[i]) {
				SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255); // Rouge
				SDL_Rect rectOut = { 5, 90 + i*60, 190, 60 };
				SDL_RenderDrawRect(renderer, &rectOut);
			}
		...
		if (jeuTermine) {
			...
			SDL_Surface* surfaceMessage = TTF_RenderText_Solid(Sans, messageFin, noir);
			SDL_Texture* Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage);
		
			SDL_Rect Message_rect;
			Message_rect.w = surfaceMessage->w;
			Message_rect.h = surfaceMessage->h;
			Message_rect.x = (1024 - Message_rect.w) / 2;
			Message_rect.y = 580;
			...
			SDL_Rect quitterBtn = {412, 400, 200, 60};
			SDL_SetRenderDrawColor(renderer, 200, 0, 0, 255);
			SDL_RenderFillRect(renderer, &quitterBtn);
		
			SDL_Surface* quitterText = TTF_RenderText_Solid(Sans, "Quitter", noir);
			SDL_Texture* quitterTex = SDL_CreateTextureFromSurface(renderer, quitterText);
		
			SDL_Rect textRect = {quitterBtn.x + 40, quitterBtn.y + 15, quitterText->w, quitterText->h};
			SDL_RenderCopy(renderer, quitterTex, NULL, &textRect);
		
			SDL_DestroyTexture(quitterTex);
			SDL_FreeSurface(quitterText);
		}
